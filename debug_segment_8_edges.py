#!/usr/bin/env python3
"""
Debug segment 8 edge assignment
"""
import numpy as np
...
print(f'Segment 8 has {len(touching_coords)} touching points')
...
for edge_name, points in edge_groups.items():
    ...
    if len(points) > 0:
        ...
        print(f'    Center point: ({center_y}, {center_x})')
        ...
        if immediate_samples:
            ...
            print(f'    Immediate samples: {len(immediate_samples)}, mean brightness: {np.mean(immediate_samples):.1f}')
        else:
            print(f'    No immediate samples!')
